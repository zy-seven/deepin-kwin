...
void ShowFpsEffect::prePaintScreen(ScreenPrePaintData &data, std::chrono::milliseconds presentTime)
{
    ...
    if (!m_scene) {
        ...
        const auto url = QUrl::fromLocalFile(QStandardPaths::locate(QStandardPaths::GenericDataLocation, QStringLiteral("deepin-kwin/effects/showfps/qml/main.qml")));
        ...
        if (!m_scene->rootItem()) {
            ...
            const auto fallbackUrl = QUrl::fromLocalFile(QStandardPaths::locate(QStandardPaths::GenericDataLocation, QStringLiteral("deepin-kwin/effects/showfps/qml/main-fallback.qml")));
